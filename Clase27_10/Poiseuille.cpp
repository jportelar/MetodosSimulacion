...
void LatticeBoltzmann::Collision(double gx, double gy){
  ...
  for(ix=0;ix<Lx;ix++)
    for(iy=0;iy<Ly;iy++){
      rho0=rho(ix,iy,false); Fx=gx*rho0; Fy=gy*rho0;Ux0=Jx(ix,iy,false,Fx)/rho0; Uy0=Jy(ix,iy,false,Fy)/rho0;
      for(i=0;i<Q;i++){
        n0=n(ix,iy,i);
