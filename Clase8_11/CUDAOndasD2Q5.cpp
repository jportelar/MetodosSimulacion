...
//data index
__device__ int d_n(int ix,int iy,int i){return (ix*Ly+iy)*Q+i;};
...
private:
  ...
  float h_w[Q]; //pesos
  int h_Vx[Q],h_Vy[Q]; //vectores de vel
  float *h_f,*h_fnew; float *d_f, *d_fnew; //func. de distribución
public:
...
void LatticeBoltzmann::Print(const char * NameFile){
  ...
  //bring back the data from the device to the host
