...
public:
  ...
  double dFuerza(double x,double y);
...
double LatticeBoltzmann::sigmaxx(int ix,int iy){
  double derivxx; int i; double rho0i,rho0f;
  rho0i=rho(ix,iy,false);
  for(i=0;i<Q;i++){
    rho0f=rho(ix+Vx[i],iy+Vy[i],false);
    derivxx+=w[i]*Vx[i]*Jx(ix+Vx[i],iy+Vy[i],false)/rho0f;
  }
  derivxx*=3;
  return -rho0i/3+eta*2*derivxx;
}
double LatticeBoltzmann::sigmayy(int ix,int iy){
  double derivyy; int i; double rho0i,rho0f;
  rho0i=rho(ix,iy,false);
  for(i=0;i<Q;i++){
    rho0f=rho(ix+Vx[i],iy+Vy[i],false);
    derivyy+=w[i]*Vy[i]*Jy(ix+Vx[i],iy+Vy[i],false)/rho0f;
  }
  derivyy*=3;
  return -rho0i/3+eta*2*derivyy;
}
double LatticeBoltzmann::sigmaxy(int ix,int iy){
  double derivxy,derivyx; int i; double rho0i,rho0f;
  rho0i=rho(ix,iy,false);
  for(i=0;i<Q;i++){
    rho0f=rho(ix+Vx[i],iy+Vy[i],false);
    derivxy+=w[i]*Vy[i]*Jx(ix+Vx[i],iy+Vy[i],false)/rho0f;
    derivyx+=w[i]*Vx[i]*Jy(ix+Vx[i],iy+Vy[i],false)/rho0f;

  }
  derivxy*=3;derivyx*=3;
  return eta*(derivxy+derivyx);
}
double LatticeBoltzmann::dFuerza(double x,double y,vector3D dA){
