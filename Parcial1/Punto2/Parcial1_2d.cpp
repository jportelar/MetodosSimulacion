...
int main(void){
  ...
  double tequilibrio=270.0;
  double omega;
  double omegamax=3.0;
  double domega=0.01;
  string datos="AmplitudFrecuencia.dat";
  ofstream archivo;

  for(omega=0.1 ; omega<omegamax ; omega +=domega){ //Se define un ciclo para correr sobre el dominio de la frecuencia//
    //InicieAnimacion();
    ...
    for(t=0, tdibujo=0 ; t<tmax ; t+=dt, tdibujo +=dt){
      ...
      if(t>=tequilibrio){ //Se extrae la amplitud de la masa 1 para el tiempo de equilibrio
	if(t=tequilibrio){
	  archivo.open(datos.c_str(), fstream::app);
	  archivo<<t<<" "<<omega<<" "<<Grano[0].Getx()<<endl;
	  archivo.close();
	  break;
	}
      }
