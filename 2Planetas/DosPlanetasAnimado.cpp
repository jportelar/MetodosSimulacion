...
void Inicie(double x0,double y0,double z0,
  ...
  friend class Colisionador; //cualquier especimen de la clase colisionador puede acceder a los datos privados de la clase cuerpo
};
...
int main(){
  ...
  double t,tmax=10*T,dt=0.01;//AQUI SE DISMINUYE POR EL MÉTODO DE INTEGRACIÓN
  double tdibujo,tcuadrado=T/500;
  int i;
  ...
  for(t=0,tdibujo=0;t<1.1*T;t+=dt,tdibujo+=dt){
    if(tdibujo>tcuadrado){
      InicieCuadro();
      for(i=0;i<N;i++) Planeta[i].Dibujese();
      TermineCuadro();tdibujo=0;
    }
  }
